...
public class TestPageProcessor implements PageProcessor {
    ...
    public TestPageProcessor(){};

    public TestPageProcessor(DelightWebmagicDao delightWebmagicDao){
        this.delightWebmagicDao = delightWebmagicDao;
    }

    private TestPageProcessor(Site site) {
        this.site = site;
    }
...
//        Spider.create(new TestPageProcessor()).addUrl("http://www.budejie.com").thread(5).run();
    ...
    {
        Spider.create(new TestPageProcessor(delightWebmagicDao)).addUrl("http://www.budejie.com").thread(5).run();
    }
}
