 ...
 */
@Component
public class VideoGaoXiaoVodPageProcessor implements PageProcessor {
    private final static Logger LOG = LoggerFactory.getLogger(VideoGaoXiaoVodPageProcessor.class);
    ...
    public VideoGaoXiaoVodPageProcessor(){};

    public VideoGaoXiaoVodPageProcessor(DelightWebmagicDao delightWebmagicDao){
        this.delightWebmagicDao = delightWebmagicDao;
    }

    private VideoGaoXiaoVodPageProcessor(Site site) {
        this.site = site;
    }
...
//        if (page.getUrl().regex("^http://www\\.budejie\\.com/pic/").match()) {
        ...
        if (page.getUrl().regex("https://www.gaoxiaovod.com/zt/gaoxiaoshipin").match()) {
            try {
                //添加页数
                page.addTargetRequests(page.getHtml().xpath("//*[@class='page']/a/@href").all());
                pageNum++;
                if(pageNum > 1)
                {
                    //模拟get请求
                    ...
                    req.setUrl("https://www.gaoxiaovod.com/zt/gaoxiaoshipin/"+pageNum+"");
                    page.addTargetRequest(req);
                    //添加每页的请求url
                    page.addTargetRequests(page.getHtml().xpath("//*[@class='box']/div/ul/li/a/@href").all());

                    //模拟get请求
                    req = new Request();
                    req.setMethod(HttpConstant.Method.GET);
                    req.setUrl("https://www.gaoxiaovod.com/v/v27824.html");
                    page.addTargetRequest(req);
                    page.putField("title",page.getHtml().xpath("//div[@class='left']/div/div[@class='play-hd']/h1/text()"));
                    page.putField("videoUrl",page.getHtml().xpath("//div[@id='video']/div/video/@src"));
...
//                        this.delightWebmagicDao.insertData(delightWebmagic);
                        LOG.info("title="+page.getResultItems().get("title").toString());
                        LOG.info("videoUrl="+page.getResultItems().get("videoUrl").toString());
                        String html = page.getHtml().toString();
                        LOG.info("html="+html);
////                    // String source = "<a title=中国体育报 href=''>aaa</a><a title='北京日报' href=''>bbb</a>";
//                    List<String> list = match(html, "video:", "src");
                        if(html.indexOf("video:") != -1)
                        {
                            String str=html.substring(html.indexOf("video:"),30);
...
//        String reg = "<" + element + "[^<>]*?\\s" + attr + "=['\"]?(.*?)['\"]?\\s.*?>";
    ...
    public static void main(String[] args) throws InterruptedException {
        Spider.create(new VideoGaoXiaoVodPageProcessor()).addUrl("https://www.gaoxiaovod.com/zt/gaoxiaoshipin").thread(5).run();
    }

    public static void task(DelightWebmagicDao delightWebmagicDao)
    {
        Spider.create(new VideoGaoXiaoVodPageProcessor(delightWebmagicDao)).addUrl("http://www.6vhao.tv/dy1/index.html").thread(5).run();
    }
}
