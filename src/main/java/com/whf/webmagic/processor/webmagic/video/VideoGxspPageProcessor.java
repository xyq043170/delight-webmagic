 ...
 */
@Component
public class VideoGxspPageProcessor implements PageProcessor {
    private final static Logger LOG = LoggerFactory.getLogger(VideoGxspPageProcessor.class);
    ...
    public VideoGxspPageProcessor(){};

    public VideoGxspPageProcessor(DelightWebmagicDao delightWebmagicDao){
        this.delightWebmagicDao = delightWebmagicDao;
    }

    private VideoGxspPageProcessor(Site site) {
        this.site = site;
    }
...
//        if (page.getUrl().regex("^http://www\\.budejie\\.com/pic/").match()) {
        ...
        if (page.getUrl().regex("http://www.gxsp5156.com/").match()) {
            try {
                //添加页数
                ...
                if(pageNum > 1)
                {
                    ...
                    req.setUrl("http://www.gxsp5156.com/index_"+pageNum+".html");
                    page.addTargetRequest(req);
                    //添加每页的请求url
                    page.addTargetRequests(page.getHtml().xpath("//*[@class='content']/article/p/a/@href").all());

                    //模拟get请求
                    req = new Request();
                    req.setMethod(HttpConstant.Method.GET);
                    req.setUrl("http://www.gxsp5156.com/play/19770.html");
                    page.addTargetRequest(req);
                    page.putField("title",page.getHtml().xpath("//div[@class='content']/header/h1/text()"));
                    page.putField("videoUrl",page.getHtml().xpath("//div[@class='v-pc']/iframe/@src"));
...
//                        this.delightWebmagicDao.insertData(delightWebmagic);
    ...
    public static void main(String[] args) throws InterruptedException {
        Spider.create(new VideoGxspPageProcessor()).addUrl("http://www.gxsp5156.com/").thread(5).run();
    }

    public static void task(DelightWebmagicDao delightWebmagicDao)
    {
        Spider.create(new VideoGxspPageProcessor(delightWebmagicDao)).addUrl("http://www.6vhao.tv/dy1/index.html").thread(5).run();
    }
}
